...
<!--end of konten-->
<!--main page-->
<!-- <form method="POST" action="simpan.php" onsubmit="return true"> -->
            ...
            <div class="page_title">
               <div class="row">
                  <div class="col-xs-3 col-sm-4 col-md-6 col-lg-7 align-self-center">
                     <h2>
                        <i class="fa fa-plus-square fa-md green_color"></i>
                        &nbsp Add receiving &nbsp;&nbsp
                        <small>
                  ...
                  <div class="col-xs-3 col-sm-4 col-md-6 col-lg-5 align-self-center text-right">
                     <i class="fa fa-calendar-week fa-md"></i>
                     ...
                     <thead class="thead">
                        <tr>
                           <th class="text-left align-middle font-weight-bold" colspan="2">
                              <i class="fa fa-box-full fa-md"></i>
                              &nbsp Select Product
                           </th>
                        </tr>
                     </thead>
                     <tbody>
                        <tr>
                           <td class="col-lg-12 col-md-8">
                              ...
                              $jsArraycopy = "var prdNamecopy = new Array();\n";
                              echo '
                              <div class="form-group">
                              <select class="form-control form-control-sm select2" name="jenis" onchange="changeValueee(this.value)" required>';
                              echo ' <option disabled="disabled" selected="selected">-- Pilih --</option>';
                              while ($row = mysqli_fetch_array($buku)) {
                                 echo '<option value="' . $row['id_barang'] . '">' . $row['id_barang'] .  '&nbsp;<b>-</b>&nbsp;' . $row['produk'] . ' ( ' . $row['produk'] . '- ' . $row['harga'] . ')</option>';
                                 $jsArraycopy .= "prdNamecopy['" . $row['id_barang'] . "'] =  {id_barang:'" . addslashes($row['id_barang']) . "',produk:'" . addslashes($row['produk']) . "'} ;\n";
                              }
                              echo '</select>';
                              ?>
                              ...
                              <input type="hidden" name="prd" id="produk">
                           </td>
                           <td>
                        ...
                        <tr>
                           <td colspan="2">
                              <div class="input-group align-items-center">
                                 ...
                                 <input type="text" name="supplier" id="supplier" value="" class="col-lg-12 col-sm-7 col-md-6 form-control mr-2" style="height: 28px">
                              </div>
                           </td>
                        </tr>
                     </tbody>
                  </table>
                  <button class="btn btn-primary mb-2 mr-2 px-2 py-2 rounded w-auto" name="simpan" style="width: 66.74px">
                     <span class="fa fa-save fa-md"></span>
                     <span>Save </span>
                  </button>
                  <button type="reset" name="reset" value="reset" class="btn btn-secondary mb-2 px-2 py-2 rounded w-auto" style="width: 66.74px">
...
if (isset($_POST['simpan'])) {
   ...
   // MENGAMBIL NAMA PRODUK BERDASARKAN ID PRODUK YANG DIPILIH
   $nama = $koneksi->query("SELECT produk FROM data_produk WHERE id_barang = '$jenis'");
   $nama_produk = $nama->fetch_assoc();
   $nama_produk = $nama_produk['produk'];
   // QUERY UNTUK MASUKKAN DATA KE TABLE barang_masuk
   $insertBarang = "INSERT INTO barang_masuk(id_barang, tanggal, nama_barang, supplier, quantity) VALUES ('$jenis', '$date', '$nama_produk', '$supplier', '$quantity')";
   // QUERY UNTUK UPDATE DATA KE TABLE data_produk
   $updateBarang = "UPDATE data_produk SET stok = stok + '$quantity' WHERE id_barang = '$jenis'";
   // JALANKAN QUERY INSERT
   $insert = $koneksi->query($insertBarang);
   // JALANKAN QUERY UPDATE
   $update = $koneksi->query($updateBarang);
   // PINDAH KE HALAMAN BARANG MASUK
